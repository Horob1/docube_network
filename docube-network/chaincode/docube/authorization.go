...
// EmitAdminAuditEvent emits a special audit event for admin actions.
// This should be called whenever an admin performs a write operation.
func EmitAdminAuditEvent(
	...
	return EmitAdminEvent(ctx, EventAdminAction, payload)
}

// EmitAdminEvent emits an admin audit event with the given payload.
func EmitAdminEvent(ctx contractapi.TransactionContextInterface, eventName string, payload AdminAuditPayload) error {

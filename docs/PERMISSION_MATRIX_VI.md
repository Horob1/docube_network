...
stateDiagram-v2
    ...
    note right of ACTIVE
        ...
        - Status: ACTIVE
        - GrantedBy: ID doc owner hoặc admin
        - GrantedAt: timestamp
    end note
    ...
    note right of REVOKED
        ...
        - Status: REVOKED
        - RevokedBy: ID doc owner hoặc admin
        - RevokedAt: timestamp
    end note
```
...
| OwnerID | Caller | GrantAccess (ID của người nhận) |
| OwnerMSP | Caller | GrantAccess (MSP của người nhận) |
| SystemUserId | Caller | GrantAccess |
| Status | Hệ thống | GrantAccess→ACTIVE, RevokeAccess→REVOKED |
| GrantedBy | Hệ thống | GrantAccess (caller.ID) |
